...
- 기존 서버가 account `0001`만 적용된 상태이므로 미적용 `0002_access_permissions.py`는 새 scope 기준으로 정리한다.
...
- 기존 `fdc-trend` scope는 `0002` seed에서 제거한다.
...
- [x] account scope seed/model/test에서 `fdc-trend`를 `l0-spider`, `l1-spider`로 교체한다.
...
- 위험: scope key 변경으로 기존 적용 DB와 불일치할 수 있다.
- 대응: 사용자가 서버 account migration이 `0001`만 적용된 상태라고 확인했으므로 `0002` seed를 새 기준으로 정리한다.
