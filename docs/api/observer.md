...
| GET | `/api/v1/observer/tkin-prevent/processes?sdwtId=...&prcGroup=...` | T/K-IN Prevent process_id 목록 |
| GET | `/api/v1/observer/tkin-prevent/step-seqs?sdwtId=...&prcGroup=...&processId=...` | T/K-IN Prevent step_seq 목록 |
| GET | `/api/v1/observer/tkin-prevent/matrix?sdwtId=...&prcGroup=...&processId=...&stepSeq=...` | T/K-IN Prevent matrix |
...
- T/K-IN Prevent 조회에서 `lineId`는 SDWT 선택까지만 사용하고, process/step/matrix 조회는 `sdwtId`와 `prcGroup`으로 대상 `station_master.ch_main`을 찾습니다.
...
```http
GET /api/v1/observer/tkin-prevent/matrix?sdwtId=S1&prcGroup=P1&processId=PROC1&stepSeq=10
```
